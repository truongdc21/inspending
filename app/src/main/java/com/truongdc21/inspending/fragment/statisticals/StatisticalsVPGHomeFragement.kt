...
import com.truongdc21.inspending.adapter.AdapterVPGStatisticals
import com.truongdc21.inspending.adapter.AdapterVPGTransaction
...
class StatisticalsVPGHomeFragement : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.apply {
            MainVMD.getDateTime.observe(viewLifecycleOwner, Observer {
                for (i in it){
                    if (i.TypeDateTime == 7 ){
                        viewPageCategories.isUserInputEnabled = false // Vô hiệu hóo cuộn với phương thức setUserInputEnabled
                    }else{
                        viewPageCategories.isUserInputEnabled = true
                    }
                }
            })
            viewPageCategories.apply {
                adapter = AdapterVPGStatisticals(childFragmentManager , lifecycle)
